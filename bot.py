...
async def ai_summarizer_receive_name(update, context):
    ...
    try:
        ...
        def add_page_number(canvas_obj, doc_obj):
            ...
            footer_text = "Generated by NovaPDF AI"
            page_text = f"  •  Page {doc_obj.page}"

            footer_size = 14

            canvas_obj.setFont(page_bold_font, footer_size)
            footer_width = canvas_obj.stringWidth(
                footer_text,
                page_bold_font,
                footer_size
            )
            ...
            page_width = canvas_obj.stringWidth(
                page_text,
                page_font,
                8
            )

            total_width = footer_width + page_width
            start_x = (A4[0] - total_width) / 2

            # Bold, green, clickable NovaPDF AI branding.
            canvas_obj.setFillColorRGB(0.086, 0.639, 0.290)
            canvas_obj.setFont(page_bold_font, footer_size)
            ...
            canvas_obj.linkURL(
                "https://t.me/novapdfassistant_bot",
                (
                    start_x - 2,
                    22,
                    start_x + footer_width + 2,
                    35
                ),
                relative=0
            )
...
async def ask_pdf_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    temp_dir = tempfile.mkdtemp()
    input_path = os.path.join(temp_dir, "active.pdf")
    ...
    try:
        ...
        await update.effective_message.reply_text(
            f"✅ {context.user_data['ask_pdf_name']} is now the active PDF.\n\n"
        ...
        logger.error(f"Ask PDF upload error: {e}")
