...
async def extract_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Format the complete AI response first, then split it
        # safely so sentences and sections are not randomly cut.
        answer_chunks = split_ai_answer(answer, 3800)

        for index, chunk in enumerate(answer_chunks):
            is_last = index == len(answer_chunks) - 1
